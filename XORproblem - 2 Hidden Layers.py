...
# Generating random weights for the second hidden layer to the output layer
output_weights = np.random.uniform(size=(hidden_layer2_size, output_layer_size))
...
for epoch in range(epochs):
    ...
    hidden_feedforward = np.dot(hidden_layer2_input, output_weights)
    ...
    for i in range(len(error)):
        ...
        if abs(error[i]) <= error_threshold:
            ...
            error_hidden2 = delta_output.dot(output_weights.T)
            ...
            output_weights += hidden_layer2_input[i].reshape(-1, 1).dot(delta_output.reshape(1, -1)) * learning_rate
...
with open('model_weights.pkl', 'wb') as file:
    pickle.dump((hidden_weights1, hidden_weights2, output_weights), file)
...
def predict(input_data, hidden_weights1, hidden_weights2, output_weights):
    ...
    output_layer_output = sigmoid(np.dot(hidden_layer2_input, output_weights))
    return output_layer_output

# Perform predictions on the testing_data
testing_predictions = predict(testing, hidden_weights1, hidden_weights2, output_weights)
