...
for epoch in range(epochs):
    ...
    for i in range(len(error)):
        ...
        if abs(error[i]) <= error_threshold:
            ...
            output_weights += (
                hidden_layer_input[i].reshape(-1, 1).dot(delta_output.reshape(1, -1))
                * learning_rate
            )
            hidden_weights += (
                inputs[i].reshape(-1, 1).dot(delta_hidden.reshape(1, -1))
                * learning_rate
            )
...
for i in range(len(testing_predictions)):
    print(
        f"Test {i + 1} - Predicted: {testing_predictions[i][0]}, Actual: {testing_ans[i]}"
    )
...
def compute_accuracy(predictions, actual):
    ...
    for i in range(len(predictions)):
        if (
            (predictions[i][0] < 0.5 and actual[i] == 0)
            or (predictions[i][0] >= 0.5 and actual[i] == 1)
        ):
